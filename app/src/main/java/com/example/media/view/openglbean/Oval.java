 ...
 * 注意使用三角形方式绘制圆时，设置的图元
 *
 * 函数 glDrawArrays(type, startIndex, count); 绘制三角形
 *
 * 其中 type 分为：
 * GL_TRIANGLE：这种理解最简单。就是每三个顶点绘制一个三角形。但这样顶点数组长度会比较大。
 * GL_TRIANGLE_STRIP：strip：带状，条状的意思，在绘制朝一个方向延展的图形时选择它。
 * GL_TRIANGLE_FAN：fan：扇形，在绘制扇形、圆形时选择它。
 *
 */
public class Oval{
